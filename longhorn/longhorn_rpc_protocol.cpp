...
int send_msg(int fd, struct message *msg)
{
    ...
    if (n != sizeof(magic_version)) {
        fprintf(stderr, "Failed to write magic version\n");
        errno = EINVAL;
        return -1;
    }
    ...
    if (n != sizeof(seq)) {
        fprintf(stderr, "Failed to write seq\n");
        errno = EINVAL;
        return -1;
    }
    ...
    if (n != sizeof(type)) {
        fprintf(stderr, "Failed to write type\n");
        errno = EINVAL;
        return -1;
    }
	...
	if (msg->data_length != 0) {
		...
		if (n != msg->data_length) {
			...
			fprintf(stderr, "Failed to write data, wrote %zd; expected %u\n", n, msg->data_length);
            errno = EINVAL;
            return -1;
		}
	}
...
int receive_msg(int fd, struct message *msg)
{
    ...
    if (n != sizeof(msg->magic_version)) {
        fprintf(stderr, "Failed to read magic version\n");
        errno = EINVAL;
        return -1;
    }
	msg->magic_version = le16toh(msg->magic_version);
    if (msg->magic_version != MAGIC_VERSION) {
        fprintf(stderr, "Wrong magic version 0x%x, expected 0x%x\n", msg->magic_version, MAGIC_VERSION);
        errno = EINVAL;
        return -1;
    }
    ...
    if (n != sizeof(msg->seq)) {
        fprintf(stderr, "Failed to read seq\n");
        errno = EINVAL;
        return -1;
    }
	...
	if (msg->data_length > 0) {
		...
		if (n != msg->data_length) {
            fprintf(stderr, "Cannot read full from fd, %u vs %zd\n", msg->data_length, n);
			free(msg->data);
            errno = EINVAL;
            return -1;
		}
